function [seps, allgroups, FEs, DeltaMtx] = gdg(fname, function_number, D, LB, UB, epsilon)
...
fsamples = feval(fname, samples',function_number) - 100 * function_number;
threshold = min(fsamples)*epsilon;
FEs = FEs+10;
    
temp = 0;
p1 = LB;
P1 = feval(fname, p1', function_number);
FEs = FEs+1;
P2_vec = zeros(D,1);
for i = 1:D
   p2 = p1;
   p2(i) = UB(i);
   P2_vec(i) = feval(fname, p2', function_number);
   FEs = FEs+1;
end
P3_vec = zeros(D,1);
for i = 1:D
   p3 = p1;
   p3(i) = temp;
   P3_vec(i) = feval(fname, p3', function_number);
   FEs = FEs+1;
end
DeltaMtx = zeros(D);
for i = 1:D-1
    for j = i+1:D
        ...
        p4(i) = UB(i);
        p4(j) = temp;
        P4 = feval(fname, p4', function_number);
        FEs = FEs+1;
