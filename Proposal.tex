...
Principally, the replacement phase considers a defined radius ($D_t$), which for simlicity is considered through the normalized euclidean distance (equation \ref{eqn:distance}), however other distances could be implemented (e.g. mahalanobis distance).
%
...
%
It is interesting to take into account that our proposal could be traslated to the multi-modal topic just setting a final distance factor, which should be strictly positive, however this trend is not considered in this work.
%
Therefore, when the initial distance factor is setted several factors should be taken into account.
%
One of them is the maximum number of evaluations, when the problem is setted with few evaluations, the initial distance factor should be lower, since the exploitation should be promoted.
%
On the other hand, when the problem is allowed with a lot evaluations, then the initial distance factor should be higher, but also should be considered the number of vectors in the population, since that them could mean 
%
