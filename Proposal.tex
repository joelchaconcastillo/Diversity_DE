...
\end{equation}
%
Therefore, at the first optimization stages are generated values near of $0.5$.
%
However, as the execution transcurs, the density function suffer a gradual transformation, thus the variance is increased.
%
It shows a bias to sample values outside the interval $[0.0, 1.0]$.
%
Where the values larger than $1.0$ are trucated, and the negative values are sampled again (until it generates a positive factor).
%
Particularly, the aim of this is to generate small $F$-values at the first optimization stages, and at the end larger $F$-values ($F=1$), in such way that the fast convergence can be avoided.
%
%
