...
Since that premature convergence is a common drawback in  \EAS{}, several variants to deal with this issue have been designed \cite{eshelman1993real}.
%
...
%
Principally, --depending on the component of the \EA{} that is modified-- these methods are classified in groups as follows \cite{Crepinsek:13}: \textit{selection-based}, \textit{population-based}, \textit{crossover/mutation-based}, \textit{fitness-based}, and \textit{replacement-based}.
%
Recently, the replacement-based strategies have attained quite good performance, therefore this section is devoted to this line.
%
One of the first techniques categorized as replacement-based are the \textit{crowding} methods.
%
One of the most popular realization is the \textit{Restricted Tournament Selection} \cite{harik1995finding} (RTS) strategy.
%
...
Similarly, in other strategies the diversity is considered to calculate a fitness value for the replacement stage.
%
...
%
In order, to calculate the contribution to diversity, the mean distance to the closest $N_{Close}$ individuals is calculated.
%
For an individual $I$, the ranking in terms of diversity is denotes ad $RD(I)$ whereas the ranking for the original cost is denoted as $RC(I)$.
%
Then, the rankings are combined to generate the biased fitness value $BF(I)$ using the Eq. \ref{eqn:biased}.
%
In each step of the replacement phase, the individual with the lowest fitness is erased and the ranks are re-calculated.
%
This process is performed until the desired population size is attained.
%
It is importat to remark that this sche requires the setting of the parameters: $N_{Close}$ and $N_{Elite}$
\begin{equation}\label{eqn:biased}
 BF(I) = RC(I) + \left(1- \frac{N_{Elite}}{N_{population}} \right) RC(I)
\end{equation}
